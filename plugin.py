...
class PluginManager():
    ...
    def _load_class(self, name, plugin_base, plugin_dict):
        def _class_exec(namespace):
            namespace.update(plugin_dict)
        logger.info("Loading plugin %s based on %s", name, plugin_base)
        plugin_module_name = f'{name}_plugin'
        plugin_module = importlib.import_module(plugin_module_name)
        plugin_class = getattr(plugin_module, name)
        self._plugins_classes[name] = types.new_class(name, bases=(plugin_base, plugin_class), exec_body=_class_exec)
        logger.info("Loaded plugin %s: %s", name, self._plugins_classes[name])

    def _load(self, name, plugin_base, plugin_dict):
        if name not in self._plugins_classes:
            self._load_class(name, plugin_base, plugin_dict)
    ...
    def _load_all(self, plugin_dir, plugin_base, plugin_dict):
        ...
        for plugin, path in self._enumerate_plugins(plugin_dir).items():
            self._load(plugin, plugin_base, plugin_dict)
